...
用法：
  第一次运行（登录）：python xhs_scraper.py --login
  之后每次抓取：     python xhs_scraper.py
  每天定时运行：     python xhs_scraper.py --schedule
"""
...
# 博主主页（只抓最近24小时内的新帖）
BLOGGERS = [
    {"name": "小万来咯", "id": "660196df000000000b00f5fb"},
    {"name": "张咋啦",   "id": "59757acd50c4b45e6e9a90df"},
]
...
async def run():
    if not os.path.exists(SESSION_FILE):
        print("❌ 未找到登录状态，请先运行：python xhs_scraper.py --login")
        return
